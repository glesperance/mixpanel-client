...
var MIXPANEL_API_HOST = 'api.mixpanel.com'
...
MixpanelClient.prototype._sendRequest = function(endpointURI, data, callback) {
  ...
  properties.token  || (properties.token  = this.APIToken                     );
  properties.time   || (properties.time   = getUnixTime()                     );
  properties.ip     || (properties.ip     = this.req.connection.remoteAddress );
  ...
  http.get(
      ...
      , path  : endPointURI + '?' + querystring.stingify(requestData)
      }
...
MixpanelClient.prototype.track = function(eventName, properties, callback) {
  ...
  compiledProperties = {
      event : eventName
    , data  : _.extend(new Object(this.superProperties), properties)
  };
...
MixpanelClient.prototype.register_once = function(properties, defaultValue) {
  ...
  for (var key in properties) {
    if (!this.superProperties.hasOwnProperty(key) || (defaultValue && this.superProperties[key] === defaultValue)) {
 ...
 *     middlewares.
 */
function MixpanelClient(APIToken, req, res) {
  this.MIXPANEL_COOKIE_NAME = 'mp' + MIXPANEL_TOKEN + '_mixpanel';
  this.APIToken             = APIToken;
  this.cookieJar            = new cookies(req, res);
  this.superProperties      = this.cookieJar.get(this.MIXPANEL_COOKIE_NAME) ? JSON.parse() : {};
}
